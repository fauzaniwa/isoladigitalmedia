...
if (isset($_GET['status'])) {
    ...
    if ($status == 'success') {
        echo "<script>alert('Berhasil menambahkan notulensi!');</script>";
    } elseif ($status == 'error') {
        echo "<script>alert('Gagal menambahkan notulensi!');</script>";
    } elseif ($status == 'success_delete') {
        echo "<script>alert('Berhasil menghapus notulensi!');</script>";
    } elseif ($status == 'error_delete') {
        echo "<script>alert('Gagal menghapus notulensi!');</script>";
    } elseif ($status == 'invalid_request') {
        echo "<script>alert('Gagal menambahkan notulensi! Invalid Request');</script>";
    } elseif ($status == 'invalid_file') {
        echo "<script>alert('File tidak sesuai (pdf)!');</script>";
    } elseif ($status == 'upload_error') {
        echo "<script>alert('File gagal terupload!');</script>";
    }
}
?>
...
<body>
        ...
        <div class="flex flex-col flex-1">
            ...
            <main class="h-full overflow-y-auto">
                <div class="container px-6 mx-auto grid">
                    ...
                    $sql = "SELECT * FROM notulensi 
    WHERE no_rapat LIKE ? OR agenda LIKE ?
    ...
    LIMIT ?";
                            ...
                            <table class="w-full whitespace-no-wrap">
                                <thead>
                                    <tr
                                        ...
                                        <th class="px-4 py-3">Nomor Rapat</th>
                                        ...
                                        <tr class="text-gray-700 dark:text-gray-400">
                                                ...
                                                <div class="flex items-center text-sm">
                                                    <div>
                                                        <p class="font-semibold">
                                                            <?php echo htmlspecialchars($row['agenda']); ?>
                                                        </p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td class="px-4 py-3 text-sm">
                                                <?php echo htmlspecialchars($row['no_rapat']); ?>
                                            </td>
                                            <td class="px-4 py-3 text-sm">
                                                <?php echo htmlspecialchars($row['tgl_rapat']); ?>
                                            </td>
                                            <td class="px-4 py-3 text-sm">
                                                <?php echo htmlspecialchars($row['total_hadir']); ?>
                                            </td>
                                            <td class="px-4 py-3 text-xs">
                                                <a class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100"
                                                    href="src/filenotulensi/<?php echo htmlspecialchars($row['file']); ?>">
                                                    File
                                                </a>
                                            </td>
                                            <td class="px-4 py-3 text-xs">
                                                <?php if ($row['link']): ?>
                                                    <a class="px-2 py-1 font-semibold leading-tight text-green-700 bg-green-100 rounded-full dark:bg-green-700 dark:text-green-100"
                                                        href="<?php echo htmlspecialchars($row['link']); ?>" target="_blank">
                                                        Link
                                                    </a>
                                                <?php else: ?>
                                                    <a
                                            ...
                                            <td class="px-4 py-3">
                                                <div class="flex items-center space-x-4 text-sm">
                                                    <!-- Edit Button -->
                                                    <a href="edit_notulensi?id=<?php echo htmlspecialchars($row['id_notulensi']); ?>"
                                                    ...
                                                    <!-- Delete Button -->
                                                    <form method="POST" action="../controllers/delete_notulensi.php"
                                                        onsubmit="return confirm('Are you sure you want to delete this item?');">
                                                        <input type="hidden" name="id"
                                                            value="<?php echo htmlspecialchars($row['id_notulensi']); ?>">
                                                        <button type="submit"
                ...
                <div x-show="isModalOpen" x-transition:enter="transition ease-out duration-150"
                    ...
                    <!-- Modal -->
                    <form action="../controllers/insert_notulensi.php" method="POST" enctype="multipart/form-data">
                        <div x-show="isModalOpen" x-transition:enter="transition ease-out duration-150"
                            ...
                            <div class="mt-4 mb-6">
                                ...
                                <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                                    ...
                                    <label class="block text-sm">
                                        ...
                                        <input
                                            class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                                            type="text" id="nomorRapat" name="nomorRapat" required
                                            placeholder="No. Notulen" />
                                    </label>
                                    ...
                                    <label class="block text-sm">
                                        ...
                                        <input
                                            class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                                            type="number" id="totalHadir" required name="totalHadir"
                                            placeholder="000" />
                                    </label>
                                    ...
                                    <label class="block text-sm">
                                        ...
                                        <input
                                            class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                                            type="link" id="link" name="link" accept="" placeholder="https://idm.com" />
                                    </label>
