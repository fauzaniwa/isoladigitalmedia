...
if (isset($_GET['status'])) {
    ...
    if ($status == 'success') {
        echo "<script>alert('Berhasil menambahkan Mutual!');</script>";
    } elseif ($status == 'error') {
        echo "<script>alert('Gagal menambahkan Mutual!');</script>";
    } elseif ($status == 'delete_success') {
        echo "<script>alert('Berhasil menghapus Mutual!');</script>";
    } elseif ($status == 'delete_failed') {
        echo "<script>alert('Gagal menghapus Mutual!');</script>";
...
// Query untuk mengambil data dari tabel mutual
$sql = "SELECT id, name, caption, tags, link_spotify, created_at, IP_ADDRESS FROM mutual";
        ...
        <table class="w-full whitespace-no-wrap">
            <thead>
                <tr
                    ...
                    <th class="px-4 py-3">Spotify Link</th>
            ...
            <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                ...
                if ($result && $result->num_rows > 0) {
                    ...
                    // Looping untuk menampilkan setiap data dari mutual
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<td class='px-4 py-3 text-sm'>" . htmlspecialchars($row['caption']) . "</td>";
                        echo "<td class='px-4 py-3 text-sm'>" . htmlspecialchars($row['tags']) . "</td>";

                        // Menampilkan link Spotify jika ada
                        if (!empty($row['link_spotify'])) {
                            $spotifyLink = htmlspecialchars($row['link_spotify']);
                            echo "<td class='px-4 py-3 text-sm'>";
                            echo "<a href='{$spotifyLink}' target='_blank' class='text-blue-500'>Listen on Spotify</a>";
                            echo "</td>";
                        } else {
                        ...
                        echo "<td class='px-4 py-3 text-sm'>" . htmlspecialchars($row['IP_ADDRESS']) . "</td>";
                        ...
                        if (trim($_SESSION['admin_role']) === 'superadmin') {
                            ...
                            echo "<form method='POST' action='../controllers/delete_mutual.php' onsubmit=\"return confirm('Are you sure you want to delete this item?');\">";
                    ...
                    $colspan = (trim($_SESSION['admin_role']) === 'superadmin') ? '8' : '7';
                    echo "<tr>";
                    echo "<td colspan='" . $colspan . "' class='px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center'>Tidak ada data mutual</td>";
                    echo "</tr>";
                }
                ?>
            </tbody>
