...
if (isset($_POST['id'])) {
    ...
    $sql = "DELETE FROM menfess WHERE id = ?";
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            ...
            header("Location: ../public/menfess.php?status=delete_success");
            exit();
        } else {
            // Redirect ke halaman sebelumnya dengan pesan error
            header("Location: ../public/menfess.php?status=delete_failed");
            exit();
        }
    } else {
        // Redirect ke halaman sebelumnya jika statement gagal disiapkan
        header("Location: ../public/menfess.php?status=delete_error");
        exit();
    }
} else {
    // Redirect ke halaman sebelumnya jika 'id' tidak ditemukan
    header("Location: ../public/menfess.php?status=no_id_provided");
    exit();
}
